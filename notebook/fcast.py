...
def test_stationary(data, col='y', spec='c'):
  '''
  Test stationary of a series using ADF and KPSS
  ...
  result.append([name, 'level', 
                 *adfuller(level, regression=spec)[:2], 
                 *kpss(level, regression=spec)[:2]])
  result.append([name, 'diff1', 
                 *adfuller(diff1)[:2], 
                 *kpss(diff1)[:2]])
...
def data_2keras(data, n_lag=12, n_forecast=4, include_dummies=False):
  """
  ...
  m = pd.get_dummies(df.index.month)
...
def model_measure(data, yhat, config):
  ...
  n_yhat, n_forecast = yhat.shape
  
  # align yhat and comput errors
  df = data.join(yhat)
  for i in range(n_forecast):
    # ignore periods with incomplete test for evaluation
    df[f'yhat{i+1}'][-n_forecast:] = np.nan 
    df[f'yhat{i+1}']  = df[f'yhat{i+1}'].shift(i+1)
    df[f'error{i+1}'] = df['y'] - df[f'yhat{i+1}']
  
  # initialize scores dataframe and compute rmse, mae, mape for each time step
  measures = ['rmse', 'mae', 'mape']
  scores = pd.DataFrame(columns = measures, index = [config_name])
  scores.index.name = 'config'
  for i in range(n_forecast):
    ys = df[['y', f'yhat{i+1}']].dropna().to_numpy()
    y_, yhat_ = ys[:,0], ys[:,1]
    scores[f'rmse{i+1}'] = [rmse(y_, yhat_)]
    scores[f'mae{i+1}']  = [mae(y_, yhat_)]
    scores[f'mape{i+1}'] = [mape(y_, yhat_)]
  
  # compute average rmse, mae, mape for all periods
  for measure in measures:
    cols = scores.columns[scores.columns.str.startswith(measure)]
    scores[measure] = scores[cols].mean(1)
  ...
  for config in model_configs:
    for i in range(n_repeat):
      try:
        ...
        scores.append(score)
        print(score[['rmse','mae','mape']].round(4).to_dict('index'))
      except:
        pass
  scores = pd.concat(scores)
  
  return scores, model_configs[scores.reset_index()['rmse'].idxmin()]
